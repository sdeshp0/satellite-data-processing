...
from core.utils import aoi_from_bounds
import math
import json

def aoi_selector():
    """
    Configurable AOI selection based on a location search
    """
    st.subheader("Area of Interest")

    location_query = st.text_input("Enter a location (city, address, landmark):", "")

    col1, col2 = st.columns(2)
    width_km = col1.number_input("AOI width (km)", min_value=1.0, max_value=200.0, value=10.0)
    height_km = col2.number_input("AOI height (km)", min_value=1.0, max_value=200.0, value=10.0)
